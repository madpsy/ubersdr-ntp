...
enum class ClockLockRefusal : std::uint8_t {
    ...
    Contested    = 4,  // window disagrees with itself: two decoded minutes
                       // name different times, or a static field carries zero
                       // winning margin. A chain short only for undecoded
                       // minutes is None (collecting) or Staleness
};
